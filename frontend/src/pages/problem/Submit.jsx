...
const Submit = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        // Backend nests the real result one level deeper: { data: { submission: {...} } }.
        // Reading response.data directly (instead of response.data.submission) always
        // returned undefined fields here, which is why verdict/test-case counts never
        // showed correctly before.
        const submissionData = response.data?.submission || {};
        const verdict = submissionData.verdict || 'pending';
        setLastSubmission({ ...submissionData, submittedCode: code });

        if (verdict === 'pending') {
          // Judge queue is asynchronous — this is the expected immediate state.
          // Poll for the real result instead of treating "pending" as a failure.
          setCodeStatus('judging');
          toast.success('Submitted — judging in progress...');
          pollSubmission(submissionData._id);
        } else {
              ...
              <div className="flex items-center space-x-4 text-sm">
                <div className="flex items-center">
                  <div className={`w-2 h-2 rounded-full mr-2 ${
                    ...
                    codeStatus === 'judging' ? 'bg-blue-500 animate-pulse' :
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                {lastSubmission.verdict === 'accepted'
                  ...
                  : lastSubmission.verdict === 'pending' || !lastSubmission.verdict
                  ? <FiClock className="text-blue-400 text-3xl animate-pulse" />
                  : <BsXCircle className="text-red-400 text-3xl" />}
                <div>
                  <h3 className={`text-xl font-bold ${
                    lastSubmission.verdict === 'accepted' ? 'text-green-300'
                    : lastSubmission.verdict === 'pending' || !lastSubmission.verdict ? 'text-blue-300'
                    : 'text-red-300'
                  }`}>
                    {lastSubmission.verdict === 'pending' || !lastSubmission.verdict
                      ? 'Judging…'
              ...
              <div className="border-t border-gray-700 pt-4">
                <div className="bg-gray-900/60 rounded-xl border border-purple-500/20 overflow-y-auto max-h-[600px]">
                  <AnalysisPanel
                    ...
                    testCasesPassed={lastSubmission.testCasesPassed || 0}
